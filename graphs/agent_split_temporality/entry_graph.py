"""
Split Temporality Agent SDK workflow entry point.

This module provides the main integration with the experimental framework,
enabling processing of clinical notes using a split temporality workflow that 
separates event identification from temporal classification.
"""
...
def configure_logging(logging_config: Dict[str, Any]) -> None:
    ...
    for module_name in ["processor", "adapter", "models"]:
        module_logger = logging.getLogger(f"graphs.agent_split_temporality.{module_name}")
            ...
            if hasattr(self.processor, "token_tracker"):
                try:
                    ...
                    logger.info(
                        ...
                        f"${token_summary['total'].get('total_cost', 0.0):.4f}"
                    )
                except Exception as e:
