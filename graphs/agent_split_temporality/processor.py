...
from .parallel_agents.workflow.note_processor import (
    ...
    process_note_with_split_temporality,
    process_all_notes_with_split_temporality,
)
...
class LLMProcessor:
    ...
    def __init__(
        ...
        # Determine provider from explicit override or model inference
        ...
        provider_type = "vLLM" if self.is_vllm else ("Ollama" if self.is_ollama else "Azure")
        logger.info(
        ...
        if self.is_vllm:
            ...
            self.configure_vllm()
            # Explicitly prevent Azure and Ollama configuration
            self.azure_provider = None
            self.azure_client = None
            self.ollama_provider = None
        elif self.is_ollama:
            ...
            # Explicitly prevent Azure configuration
            self.azure_provider = None
            self.azure_client = None
            self.vllm_provider = None
        else:
            ...
            # Explicitly prevent Ollama and vLLM configuration
            self.ollama_provider = None
            self.vllm_provider = None
        ...
        # Initialize agent IO logger for real-time agent monitoring
    ...
    def _init_agent_logger(self):
        """Initialize agent IO logger for real-time monitoring of agent interactions."""
        try:
            from .agent_io_logger import init_io_logger, io_logger
            
            # Check if agent logger is already initialized (e.g., by debug_note.py)
            if io_logger is not None:
                logger.info("Agent IO Logger already initialized - using existing instance")
                self.agent_io_logger = io_logger
                return
            
            # Create logs directory
            log_dir = "logs/agent_io"
            os.makedirs(log_dir, exist_ok=True)
            
            # Initialize the logger with monkey patching
            self.agent_io_logger = init_io_logger(log_dir)
            
            logger.info(f"Agent IO Logger initialized - logs will be saved to {log_dir}")
            logger.info("Real-time agent monitoring enabled - agent interactions will be logged to console")
            
        except Exception as e:
            logger.warning(f"Failed to initialize agent IO logger: {e}")
            self.agent_io_logger = None
    ...
    async def process_single_note(self, note_input: NoteInput) -> NotePrediction:
        ...
        try:
            from .parallel_agents.workflow.note_processor import process_note_with_split_temporality
            ...
            if self.is_vllm:
                ...
                if not self.parameters:
                    self.parameters = {}
                self.parameters['provider_config'] = self.parameters.get('provider_config', {})
            ...
            result = await process_note_with_split_temporality(
                note_input,
                self.model_config_key,
                provider,
                None,  # request_id will be generated
                self.token_tracker,  # Pass the token tracker instance
                None,  # event_types (will use defaults)
                self.parameters,  # provider_config
                self.prompt_variant,  # Pass the prompt variant for ablation studies
            )
            ...
            # Get the actual model name used (not the config key)
            # Find the most used model from the token tracker summary
            primary_model_name = None
            max_tokens = 0
            for model_name, usage in token_usage_summary.get("by_model", {}).items():
                if usage["total_tokens"] > max_tokens:
                    max_tokens = usage["total_tokens"]
                    primary_model_name = model_name
            
            # Use the actual model name for cost rates, fallback to config key if not found
            model_for_cost = primary_model_name if primary_model_name else self.model_config_key
            cost_rates = get_model_cost_rates(model_for_cost)
            ...
            prediction.token_usage = {
                ...
                "model": model_for_cost,  # Use the actual model name, not config key
    ...
    def process_batch(self, notes: List[NoteInput]) -> List[NotePrediction]:
        ...
        try:
            ...
            # Create a batch-specific Azure client for thread safety
            batch_azure_client = self._create_batch_client()
            ...
            if loop.is_running():
                ...
                async def run_process():
                    try:
                        result = await self.process_all_notes(
                            notes, openai_client=batch_azure_client
                        )
                        future.set_result(result)
                    except Exception as e:
                        future.set_exception(e)
                ...
                # The loop is not running, use run_until_complete
                results = loop.run_until_complete(
                    self.process_all_notes(notes, openai_client=batch_azure_client)
                )
    ...
    def _create_batch_client(self):
        """
        Create a client for batch processing. Returns None for Ollama or vLLM models.

        Returns:
            Azure client or None
        """
        if self.is_ollama or self.is_vllm:
            # Ollama and vLLM models don't need a separate client for batch processing
            return None
        elif self.azure_client:
            try:
                # Create a fresh Azure client for this batch with enhanced retry settings
                # Use higher retry limits for o3-mini model config
                max_retries = 10 if "o4-mini" in self.model_config_key else 6
                timeout = 240.0 if "o4-mini" in self.model_config_key else 180.0

                logger.debug(
                    f"Creating batch client with model config {self.model_config_key}: max_retries={max_retries}, timeout={timeout}"
                )
                ...
                logger.debug(
                    f"Created batch-specific Azure client with {max_retries} retries and {timeout}s timeout"
                )
                return batch_client
            except Exception as e:
                logger.warning(f"Failed to create batch client: {str(e)}")
                # Fall back to the instance client
    ...
    def _configure_logging(self, logging_config: Dict[str, Any]) -> None:
        ...
        for module_name in ["parallel_agents", "workflow", "models", "utils"]:
            module_logger = logging.getLogger(
                f"graphs.agent_split_temporality.parallel_agents.{module_name}"
            )
            module_logger.setLevel(log_level)

        logger.debug(f"Processor logging configured with level: {log_level_str}")

    def save_agent_logs(self, output_file: Optional[str] = None) -> Optional[str]:
        """
        Save agent IO logs to a file.
        
        Args:
            output_file: Optional path to save logs to
            
        Returns:
            Path to saved file, or None if logger not initialized
        """
        if self.agent_io_logger:
            return self.agent_io_logger.save_entries(output_file)
        return None

    def cleanup(self):
        """Cleanup resources and restore original Runner.run method."""
        try:
            # Save agent logs before cleanup
            if self.agent_io_logger:
                log_file = self.save_agent_logs()
                logger.info(f"Agent logs saved to: {log_file}")
                
            # Restore original Runner.run method
            from .agent_io_logger import restore_runner
            restore_runner()
            
            logger.info("Agent IO logger cleanup complete")
        except Exception as e:
