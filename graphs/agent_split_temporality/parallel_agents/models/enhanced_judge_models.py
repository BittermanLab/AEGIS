...
class EventIdentificationResult(BaseModel):
    """Result from a single event identifier agent (split workflow)."""

    identifier_id: str
    event_present: bool = Field(
        description="Whether the event is present in the note"
    )
    reasoning: Optional[str] = Field(
        default=None,
        description="Reasoning for the identification of the event",
    )
...
class TemporalityResult(BaseModel):
    """Result from a single temporality classifier agent (split workflow)."""

    classifier_id: str
    past_events: List[str] = Field(
        default_factory=list, description="Past events identified by the classifier"
    )
    current_events: List[str] = Field(
        default_factory=list, description="Current events identified by the classifier"
    )
    reasoning: Optional[str] = Field(
        default=None,
        description="Reasoning for the temporal classification",
    )
    evidence_snippets: Optional[List[str]] = Field(
        default=None,
        description="Exact string match for any evidence snippets as a list of quotes",
...
class AggregatedEventIdentification(BaseModel):
    """Aggregated event identification results after judge evaluation (split workflow)."""

    event_present: bool = Field(
        description="Whether the event is present in the note (judged)"
    )
    reasoning: Optional[str] = Field(
        default=None, description="Reasoning for the identification of the event"
    )
    evidence_snippets: Optional[List[str]] = Field(
        default=None,
...
class AggregatedTemporality(BaseModel):
    """Aggregated temporal classification results after judge evaluation (split workflow)."""
    ...
    reasoning: Optional[str] = Field(
        default=None, description="Reasoning for the temporal classification"
    )
    evidence_snippets: Optional[List[str]] = Field(
        default=None,
        description="Aggregated exact string match for any evidence snippets as a list of quotes",
