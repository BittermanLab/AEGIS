...
def get_terms_only(category: str) -> List[str]:
    """
    Get just the term names for a specific category (no definitions or grades).
    Used for identification and temporality classification to reduce token usage.

    Args:
        category: The symptom category to load

    Returns:
        List[str]: A list of term names for the category
    """
    logger.debug(f"Getting terms only for category: {category}")
    
    try:
        data = get_ctcae_subset(category)
        terms = []
        
        for cat, terms_data in data.items():
            if isinstance(terms_data, str):
                # Handle fallback format
                terms.append(cat)
            else:
                # Handle full format - just get the term names
                terms.extend(list(terms_data.keys()))
        
        # Also ensure the main category term is included
        category_term = category.capitalize()
        if category_term not in terms:
            terms.append(category_term)
        
        logger.debug(f"Found {len(terms)} terms for {category}")
        return terms
    except Exception as e:
        logger.error(f"Error getting terms only for {category}: {str(e)}")
        raise


def get_terms_and_grades_only(category: str) -> Dict[str, Dict[str, str]]:
    """
    Get terms and their grades only (no definitions).
    Used for grading agents to reduce token usage.

    Args:
        category: The symptom category to load

    Returns:
        Dict[str, Dict[str, str]]: A dictionary mapping terms to their grades only
    """
    logger.debug(f"Getting terms and grades only for category: {category}")
    
    try:
        full_data = get_terms_definitions_and_grades(category)
        grades_only = {}
        
        for term, details in full_data.items():
            grades_only[term] = {
        ...
        logger.error(f"Error getting terms and grades for {category}: {str(e)}")
        raise
