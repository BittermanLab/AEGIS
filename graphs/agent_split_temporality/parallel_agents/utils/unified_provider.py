"""
Unified model provider factory that supports Azure OpenAI, local Ollama models, and vLLM models.
...
def get_model_for_name(
    ...
    if is_vllm_model(model_name):
        ...
        vllm_endpoint = provider_config.get("vllm_endpoint") or os.getenv("VLLM_BASE_URL")
    ...
    def get_model(self, model_name: str) -> Model:
        ...
        if is_vllm_model(model_name):
            ...
            if not self.vllm_provider:
                vllm_endpoint = self.provider_config.get("vllm_endpoint") or os.getenv("VLLM_BASE_URL")
                self.vllm_provider = VLLMModelProvider(base_url=vllm_endpoint)

            # Get and return the model
            return self.vllm_provider.get_model(model_name)
        elif is_ollama_model(model_name):
